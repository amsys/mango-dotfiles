#!/usr/bin/env bash
# Dark/light toggle for waybar (design ported from end-4/dots-hyprland). State lives in gsettings,
# same place switchwall.sh reads/writes it. --toggle re-runs the full color pipeline
# via switchwall.sh --noswitch (matugen + waybar reload) rather than duplicating it here.
set -u
