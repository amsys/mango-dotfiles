# shellcheck shell=sh
# Shared tooltip vocabulary for the ironbar modules that emit JSON.
#
...
# claudebar draws its own shapes from its own copy of this vocabulary —
# correction, checked directly: claudebar is a bash script (AUR package
# `claudebar`), not Rust. mango-bard's own tooltip.rs (T6c) ports this same
# vocabulary a third time, for its native claudebar-usage popup.
#
...
# per-cell grading existed. claudebar draws its own shapes from its own bash
# copy of this vocabulary (see the module doc comment above).
