#!/usr/bin/env bash
# Clock + world clocks + calendar, as two ironbar modules.
#
...
# string — no meters for the world-clock grid either.
#
...
#   clock.sh test         assert the timezone, face and calendar arithmetic
#
# The pomodoro that used to live here moved to mango-bard's pomo.rs (T7c) —
# see IRONBAR.md and /home/martin/.claude/plans/
# read-focus-md-implement-all-mossy-widget.md. waybar has been retired
# (T8 cutover); the pomodoro was never ported in parallel, so there was
# only ever one timer, one implementation.
set -u
...
# than trading it for a cheaper poll.
TIP_CACHE="${XDG_RUNTIME_DIR:-/tmp}/waybar-clock-tip"
if ! tip_stale "$TIP_CACHE" "$TIME"; then
