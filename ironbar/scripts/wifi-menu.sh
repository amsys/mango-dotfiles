#!/bin/sh
# Wi-Fi quick-connect menu for ironbar's wifi module (on-click).
...
notify() { notify-send -a mango-bard "Wi-Fi" "$1"; }
...
# No separate net-refresh poke on join/disconnect/radio-toggle: those are
# themselves real `nmcli` events, and net.rs's `nmcli monitor` child already
# regrades from them (IRONBAR.md T3) — the RTMIN nudge this used to need
# is gone with waybar's own poll-driven modules.
