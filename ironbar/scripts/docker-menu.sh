#!/bin/sh
# Docker quick-actions menu for ironbar's docker module (on-click).
...
notify() { notify-send -a mango-bard "Docker" "$1"; }

# On the trap so a cancelled menu is covered too, same as every other
# menu's own poke. docker.rs's filtered `docker events` child (IRONBAR.md
# T6b) already regrades on the real start/stop/die event this menu's own
# actions produce — this is a belt-and-braces catch-up poke, not the only
# path to a fresh pill.
trap 'mango-bard refresh docker 2>/dev/null' EXIT INT TERM
...
MENU=""
if [ "$N" -gt 0 ]; then
	MENU=$(printf '%s\n' "$ORDERED" | while IFS='|' read -r name state status _ _ _; do
...
if [ "$IDX" -ge "$N" ]; then
	case $((IDX - N)) in
	1) OUT=$("$DOCKER" ps -q | xargs -r "$DOCKER" stop 2>&1) || notify "Stop all failed: $OUT" ;;
